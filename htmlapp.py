...
def results() :
    ...
    # Choice2-- using smile representation
        ...
        return render_template("chemdatafile.html", name=molecule_name, s_rep=result[0][0],a_val=result[0][1], b_val=result[0][2], c_val=result[0][3], mu_val=result[0][4], homo_val=result[0][5], lumo_val=result[0][6], gap_val=result[0][7], zpve_val=result[0][8], u298_val=result[0][9], h298_val=result[0][10],g298_val=result[0][11], cv_val=result[0][12] )
    # Choice3-- using molecular formula
    elif request.form["choice"] == "choice3" :
        return render_template(str(molecule_name)+"_fresh_new.html")
...
#if __name__ == '__main__':
    #app.run(port=5000,host='depws.iitkgp.ernet.in')
